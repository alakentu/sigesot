...
class IonAuthModel
{
	...
	public function deactivate(int $id = 0): bool
	{
		...
		if (!$id) {
			...
			return false;
		} else if ((new IonAuth)->loggedIn() && $this->user()->row()->id === $id) {
	...
	public function changePassword(string $identity, string $old, string $new): bool
	{
		...
		$query = $this->db->table($this->tables['users'])->select('id, password')
	...
	public function login(string $identity, string $password, bool $remember = false): bool
	{
		...
		$query = $this->db->table($this->tables['users'])
			->select($this->identityColumn . ', email, ' . $this->tables['users'] . '.id, ip_address, password, active, last_login, first_name, middle_name, first_last_name, second_last_name, genders.gender_name as gender, nationality.name as nationality, photo, company, phone')
			->join('genders', 'genders.id = users.gender')
			->join('nationality', 'nationality.id = users.nationality')
			->where($this->identityColumn, $identity)
			->limit(1)
			->orderBy('id', 'desc')
			->get();
	...
	public function getLastAttemptIp(string $identity)
	{
		if ($this->config->trackLoginAttempts && $this->config->trackLoginIpAddress) {
			$this->db->table($this->tables['login_attempts'])
				->select('ip_address')
				->where('login', $identity)
				->orderBy('id', 'desc');

			$qres = $this->db->table($this->tables['login_attempts'])->get(1, 0);
	...
	public function getLastLogin($time): string
	{
		// Calculate the time difference in seconds
		$difference = time() - $time;

		// Convert the difference to months, days, hours and minutes
		$months = floor($difference / (30 * 24 * 60 * 60));
		$days = floor($difference / (24 * 60 * 60));
		$hours = floor($difference / (60 * 60));
		$minutes = floor($difference / 60);

		// Create the message based on the time difference
		if ($months > 0) {
			$message = lang('Site.yourLastVisit') . ' ' . ($months == 1 ? lang('Site.lastVisitMonth', [$months]) : lang('Site.lastVisitMonths', [$months]));
		} elseif ($days > 0) {
			$message = lang('Site.yourLastVisit') . ' ' . ($days == 1 ? lang('Site.lastVisitDay', [$days]) : lang('Site.lastVisitDays', [$days]));
		} elseif ($hours > 0) {
			$message = lang('Site.yourLastVisit') . ' ' . ($hours == 1 ? lang('Site.lastVisitHour', [$hours]) : lang('Site.lastVisitHours', [$hours]));
		} else {
			$message = lang('Site.yourLastVisit') . ' ' . ($minutes == 1 ? lang('Site.lastVisitMinute', [$minutes]) : lang('Site.lastVisitMinutes', [$minutes]));
		}
	...
	{
		if (!array_key_exists($template, config('Config\\Validation')->templates)) {
		...
		return view(config('Config\\Validation')->templates[$template], ['errors' => $errors]);
	}
