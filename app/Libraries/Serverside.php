...
class Serverside
{
    ...
    static function limit($request, $columns)
    {
        ...
        $lenght = ($request['length'] != '' && $request['length'] != '-1') ? $request['length'] : 10;

        if ($start && $lenght) {
            $limit = "LIMIT " . $start . " OFFSET " . $lenght;
        }
    ...
    static function sql_connect()
    {
        ...
        $driver = ($db->DBDriver == 'Postgre') ? 'pgsql' : 'mysql';
        ...
        try {
            $db = new \PDO($dsn, $db->username, $db->password, $options);
        } catch (\PDOException $e) {
            self::fatal(
                "Se produjo un error al conectarse a la base de datos. El error reportado por el servidor fue: " . $e->getMessage()
            );
        }
