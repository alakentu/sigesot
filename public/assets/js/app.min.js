/*!
	* Template Name: Sigesot - Admin & Dashboard Template
	...
	function pluginData() {
		...
		Array.from(flatpickrExamples).forEach(function (item) {
			if (item.getAttribute("data-provider") == "flatpickr") {
				...
				// Obtener idioma del navegador (código de 2 letras)
				var userLang = (navigator.language || navigator.userLanguage).substring(0, 2);

				// Verificar si el idioma está disponible en flatpickr
				if (flatpickr.l10ns[userLang]) {
					flatpickr.localize(flatpickr.l10ns[userLang]);
				} else {
					// Idioma por defecto si no está disponible
				...
				if (splitItem.insertAdjacentHTML) {
					splitItem.insertAdjacentHTML(
						...
						<a class="nav-link" href="#sidebarMore" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="sidebarMore">\
							<i class="bi bi-briefcase"></i> <span data-key="t-more">' + extraMenuName + '</span>\
						</a>\
...
(function ($) {
	...
	var $window = $(window),
		$body = $(document.body),
		$matches = document.body.className.match(/(^|\s)view-(\w+)(\s|$)/),
	...
	$(document).on('click', ':checkbox:not([name="groups[]"])', function () {
		var checkbox = $(this);
	...
	$(document).on('click', ':checkbox[name="groups[]"]', function () {
		var checkbox = $(this),
			checked = checkbox.is(':checked'),
			d_check = checkbox.data('checking');
		if (checked) {
			// Restaurar el valor original
			checkbox.val(d_check).attr('checked', true);
		} else {
			// Vaciar el value
			checkbox.val('').attr('checked', false);
		}
	});

	$(document).on('change', ':checkbox:not([name="groups[]"])', function () {
	...
	$(document).on('check uncheck', ':checkbox:not([name="groups[]"])', function (e) {
		var checked = (e.type === 'check');
		$(this).data('ischecked', checked).attr('checked', checked);
	});
	...
	$(function () {
		if ($matches[2] === 'users') {
			var form = document.getElementById('modalNewUser');
			var submitBtn = document.querySelector("button[name=submit]");

			// Validación en tiempo real para contraseñas
			var $password = $(form).find('input[name="password"]');
			var $passwordConfirm = $(form).find('input[name="password_confirm"]');

			if ($password.length && $passwordConfirm.length) {
				$passwordConfirm.on('input', function () {
					if ($password.val() !== $passwordConfirm.val()) {
						//$passwordConfirm[0].setCustomValidity('Las contraseñas no coinciden');
						$("#check-confirm .invalid-tooltip").text("Confirmar contraseña no coincide con la contraseña").show();
					} else {
						//$passwordConfirm[0].setCustomValidity('');
						$("#check-confirm .invalid-tooltip").text("").hide();
					}
				});
			}

			$('#modalNewUser').on('submit', function (e) {
				e.preventDefault();
				//var $submitBtn = $('#addNewUser');
				...
				var $checkboxes = $(form).find('input[name="groups[]"]');
				var $groupError = $("#groupError");
				var $checkboxGroup = $checkboxes.length > 0 ? $checkboxes.first().closest('.position-relative') : null;

				var atLeastOneChecked = $checkboxes.length === 0;
				...
				if (!atLeastOneChecked && $checkboxGroup) {
					formIsValid = false;
					$checkboxGroup.addClass('was-validated');
					$groupError.show();
				} else if ($groupError.length) {
					$groupError.hide();
				}
						...
						success: function (data) {
							...
							if ($.isEmptyObject(data.error)) {
								if (data.code == 1) {
									...
									// Redirigir a la página de listado con parámetro de éxito
			...
			$('#usersTable').on('click', '#change_status', function (e) {
				e.preventDefault();
